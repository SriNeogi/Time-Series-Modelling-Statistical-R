...
#####d_mat################################ Test Train Split ############################
t=3 #size of test set
#Note: This is a matrix not a time series object
x_train = d_mat[,1:(c-t)]
...
for(i in 1:n)
{
  ets[i,] = round(forecast(ets(ts(x_train[i,], frequency=12),model='ZZZ'), h=t)$mean)
  ar[i,] = round(forecast(auto.arima(ts(x_train[i,],frequency=12)), h=t)$mean)
  theta[i,] = round(thetaf(ts(x_train[i,], frequency=12),h=t)$mean)
  stlf[i,] = stlf(ts(x_train[i,], frequency=12), h=t)$mean
  tbats[i,]= forecast(tbats(ts(x_train[i,], frequency=12),use.box.cox=TRUE,
                            use.trend=TRUE,
                            use.damped.trend=TRUE,
                            biasadj=TRUE), h=t)$mean
  
  knn[i,] = round(knn_forecasting(ts(x_train[i,], frequency=12), h=t, k=6, msas = "recursive",cf="mean")$prediction)
...
print(i)
autoplot(ts(ets1[1,], frequency=12))
